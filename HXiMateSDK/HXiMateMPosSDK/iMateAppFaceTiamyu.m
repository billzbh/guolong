...
- (NSString *)deviceSerialNumber
{
    if (self.iMateAccessory == nil)
        return nil;
    return [self.iMateAccessory serialNumber];
}
...
-(void)startSearchBLE:(int)timeout
{
    ...
    if (!_isSearching) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            _isSearching = YES;
            HuaxinZhinengService *tyPinpadService = [[HuaxinZhinengService alloc] init];
            
            NSArray *deviceArray = [tyPinpadService deviceInit:2];
            usleep(500000);
            NSMutableArray *deviceName = [NSMutableArray arrayWithCapacity:10];
            ...
            while ([[NSDate date] timeIntervalSince1970] < timeSeconds) {
                ...
                if (deviceArray && deviceArray.count) {
                    for (CBPeripheral *peripheral in deviceArray) {
                        ...
                        {
                            if (peripheral.name!=nil &&![deviceName containsObject:peripheral.name]) {
                                [deviceName addObject:peripheral.name];
                                //上报TY的设备
                                if ([_delegate respondsToSelector:@selector(MPOSDelegateFoundAvailableDevice:DeviceCBUUID:)] )
                                {
                                    [_delegate MPOSDelegateFoundAvailableDevice:peripheral.name DeviceCBUUID:[peripheral.identifier UUIDString]];
                                }
                            }
                        }
                    }
                }
                deviceArray = [tyPinpadService deviceInit:2];
                usleep(100000);
            }
            tyPinpadService = nil;
            _isSearching = NO;
