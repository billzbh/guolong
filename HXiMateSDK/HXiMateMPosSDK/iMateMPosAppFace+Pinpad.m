...
static int sg_pinpadModel = PINPAD_MODEL_TIANYU;
...
//        self.pinPad = [iMateM35PinPad imatePinPad];
    if (pinpadModel == PINPAD_MODEL_TIANYU)
        self.pinPad = [iMateTyPinPad imatePinPad];
}

-(void)BindName:(NSString*)TYname
{
    if (sg_pinpadModel == PINPAD_MODEL_TIANYU)
        [self.pinPad BindName:TYname];
}
