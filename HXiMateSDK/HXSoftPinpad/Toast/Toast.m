...
- (id)initWithText:(NSString *)text_{
    if (self = [super init]) {
		...
		if ([[[UIDevice currentDevice] systemVersion] floatValue] > 7.0f)//IOS 7.0 以上
		{
        ...
        UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, size.width	+ 12, size.height + 12)];
...
+ (void)showWithText:(UIView *)view text:(NSString *)text duration:(CGFloat)duration{
	//[Toast showWithText:text duration:duration];
