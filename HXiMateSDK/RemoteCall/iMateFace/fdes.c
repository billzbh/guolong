...
int DES3_CBCUpdate2(DES3_CBC_CTX *context, unsigned char *output, unsigned char *input){
	...
	len = (int)strlen((const char*)input);
	if(context->encrypt == 0) {
		// 解密
		if(len % 16)
			return(RE_LEN);
...
void scrunch(UINT4 *into, unsigned char *outof){
	...
	*into   |= (UINT4)(*outof++ & 0xffL) << 16;
	*into   |= (UINT4)(*outof++ & 0xffL) << 8;
	*into++ |= (UINT4)(*outof++ & 0xffL);
	*into    = (UINT4)(*outof++ & 0xffL) << 24;
	*into   |= (UINT4)(*outof++ & 0xffL) << 16;
	*into   |= (UINT4)(*outof++ & 0xffL) << 8;
	*into   |= (UINT4)(*outof   & 0xffL);
...
static void cookey(UINT4 *subkeys, UINT4 *kn, int encrypt){
	...
	for (i = 0; i < 16; i++, raw1++) {
		...
		*cooked   |= (UINT4)(*raw0 & 0x00000fc0L) << 10;
		*cooked   |= (UINT4)(*raw1 & 0x00fc0000L) >> 10;
		*cooked++ |= (UINT4)(*raw1 & 0x00000fc0L) >> 6;
		*cooked    = (UINT4)(*raw0 & 0x0003f000L) << 12;
		*cooked   |= (UINT4)(*raw0 & 0x0000003fL) << 16;
		*cooked   |= (UINT4)(*raw1 & 0x0003f000L) >> 4;
		*cooked   |= (UINT4)(*raw1 & 0x0000003fL);
		cooked += increment;
	}
}
