...
{
    unsigned char sendBytes[6 + uiLen];
    
    sendBytes[0] = 0x68;
    sendBytes[1] = 0x04;
    sendBytes[2] = uiOffset / 256;
    sendBytes[3] = uiOffset % 256;
    sendBytes[4] = uiLen / 256;
    sendBytes[5] = uiLen % 256;
    memcpy(sendBytes + 6, pBuf, uiLen);
    
    int ret = syncCommonEx(sendBytes, 6 + uiLen, NULL, NULL, 1);
    return ret;
}
...
// 返    回：0          : 成功
//           其它       : 失败
uint _uiRcMifAuthEx(uchar ucSecNo, uchar ucKeyAB, uchar *psKey)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(100);
    uiDataLen = sprintf(psFunCall, "%u(%u,%u,e008",FUNC_uiMifAuth, ucSecNo, ucKeyAB);
    memcpy(psFunCall + uiDataLen, psKey, 8);
    uiDataLen += 8;
    uiDataLen += sprintf(psFunCall + uiDataLen, ")");
...
uchar _ucRcSLE4442_ChkCodeEx(uchar *sSecurityCode)
{
    ...
    char *psFunCall = malloc(50);
    uiDataLen = sprintf(psFunCall, "%u(e008",FUNC_SLE4442_ChkCode);
...
uchar _ucRcSLE4428_ChkPSCEx(uchar *sSecurityCode)
{
    ...
    char *psFunCall = malloc(50);
    uiDataLen = sprintf(psFunCall, "%u(e008",FUNC_SLE4428_ChkPSC);
    memcpy(psFunCall + uiDataLen, sSecurityCode, 8);
    uiDataLen += 8;
    uiDataLen += sprintf(psFunCall + uiDataLen,")");
    uiRet = iDoRemoteFunc((uchar*)psFunCall, uiDataLen, 2);
...
uchar _ucRcAT102_ChkSecurityCodeEx(uchar *psSC)
{
    ...
    char *psFunCall = malloc(50);
    uiDataLen = sprintf(psFunCall, "%u(e008",FUNC_AT102_ChkSecurityCode);
...
uchar _ucRcAT102_EraseAppEx(uchar ucArea, uchar ucLimited, uchar *psEraseKey)
{
    ...
    if (psEraseKey == NULL)
        ...
        uiDataLen = sprintf(psFunCall, "%u(%u,%u,e008", FUNC_AT102_EraseApp, ucArea, ucLimited);
...
uchar _ucRcAT1604_ChkSCEx(uchar *SC)
{
    ...
    char *psFunCall = malloc(50);
    uiDataLen = sprintf(psFunCall, "%u(e008",FUNC_AT1604_ChkSC);
...
uchar _ucRcAT1604_ChkSCnEx(uchar Area,uchar *SCn)
{
    ...
    char *psFunCall = malloc(50);
    uiDataLen = sprintf(psFunCall, "%u(%u,e008",FUNC_AT1604_ChkSCn, Area);
...
uchar _ucRcAT1604_ChkEZnEx(uchar Area,uchar *EZn)
{
    ...
    char *psFunCall = malloc(50);
    uiDataLen = sprintf(psFunCall, "%u(%u,e008",FUNC_AT1604_ChkEZn, Area);
...
uchar _ucRcAT1608_VerifyPasswordEx(uchar Index,uchar *Password)
{
    ...
    char *psFunCall = malloc(50);
    uiDataLen = sprintf(psFunCall, "%u(%u,e008",FUNC_AT1608_VerifyPassword, Index);
...
uchar _ucRcAT1608_AuthEx(uchar *Gc)
{
    ...
    char *psFunCall = malloc(100);
    uiDataLen = sprintf(psFunCall, "%u(e016",FUNC_AT1608_Auth);
...
uint _uiRcGenCommKey(uchar ucMasterKeyId, uchar *psRandom)
{
    uint uiDataLen, uiRet;
...
uint _uiRcEsamGetCardRandom(uint uiRandLen, uchar *psRandom)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(100);
    uiDataLen = sprintf(psFunCall, "%u(%u, b000)",FUNC_uiEsamGetCardRandom, uiRandLen);
    
    uiRet = iDoRemoteFunc((uchar*)psFunCall, uiDataLen, 5);
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("uiGenCommKey _uiRcEsamGetCardRandom ret = %d", uiRet);
        return 99;
    }
    memcpy(psRandom, psGetParaBuf(3), uiRandLen);
    return iGetParaInt(0);
}

uint _uiRcEsamExternalAuth(uchar ucMasterKeyId, uchar *psAuthData)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(100);
    uiDataLen = sprintf(psFunCall, "%u(%u, b008",FUNC_uiEsamExternalAuth, ucMasterKeyId);
    memcpy(psFunCall + uiDataLen, psAuthData, 8);
    uiDataLen += 8;
    uiDataLen += sprintf(psFunCall + uiDataLen, ")");
...
uint _uiRcEsamReloadKey(uchar *psInData)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(100);
    uiDataLen = sprintf(psFunCall, "%u(b028",FUNC_uiEsamReloadKey);
    memcpy(psFunCall + uiDataLen, psInData, 28);
    uiDataLen += 28;
    uiDataLen += sprintf(psFunCall + uiDataLen, ")");
    ...
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("uiGenCommKey uiEsamReloadKey ret = %d", uiRet);
        return 99;
    }
    return iGetParaInt(0);
}

uint _uiRcEsamDownloadMasterKey(uint uiDataType, uint uiMasterKeyIndex, uchar *psMasterKey, uint uiKeyLen)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(100);
    uiDataLen = sprintf(psFunCall, "%u(%u, %u, b%03u",
                        FUNC_uiEsamDownloadMasterKey, uiDataType, uiMasterKeyIndex, uiKeyLen);
    memcpy(psFunCall + uiDataLen, psMasterKey, uiKeyLen);
    uiDataLen += uiKeyLen;
    uiDataLen += sprintf(psFunCall + uiDataLen, ", %u)", uiKeyLen);
    
    uiRet = iDoRemoteFunc((uchar*)psFunCall, uiDataLen, 5);
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("uiEsamDownloadMasterKey iDoRemoteFunc ret = %d", uiRet);
        return 99;
    }
    return iGetParaInt(0);
}

uint _uiRcEsamDownloadWorkingKey(uint uiDataType, uint uiMasterKeyIndex, uint uiWorkingKeyIndex, uchar *psWorkingKey, uint uiKeyLen)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(100);
    uiDataLen = sprintf(psFunCall, "%u(%u, %u, %u, b%03u",FUNC_uiEsamDownloadWorkingKey, uiDataType, uiMasterKeyIndex, uiWorkingKeyIndex, uiKeyLen);
    memcpy(psFunCall + uiDataLen, psWorkingKey, uiKeyLen);
    uiDataLen += uiKeyLen;
    uiDataLen += sprintf(psFunCall + uiDataLen, ", %u)", uiKeyLen);
    ...
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("uiEsamDownloadWorkingKey iDoRemoteFunc ret = %d", uiRet);
        return 99;
    }
    return iGetParaInt(0);
}

uint _uiRcEsamCalPinBlock(uint uiDataType, char *pszCardNo, uint uiMasterKeyIndex, uint uiWorkingKeyIndex, uchar *psPin, uint uiPinLen, uchar *psPinBlock)
{
    uint uiDataLen, uiRet, uiCardNoLen;
    char szPin[30], szCardNo[40];
    char *psFunCall = malloc(300);
    
    uiCardNoLen = 0;
    memset(szCardNo, 0, sizeof(szCardNo));
    if (pszCardNo && strlen(pszCardNo)) {
        uiCardNoLen = (uint)strlen(pszCardNo);
        strcpy(szCardNo, pszCardNo);
    }
    
    memset(szPin, 0 ,sizeof(szPin));
    memcpy(szPin, psPin, uiPinLen);
    
    
    uiDataLen = sprintf(psFunCall, "%u(%u, s%03u%s, %u, %u, b%03u%s, %u, b000)",
                        FUNC_uiEsamCalPinBlock, uiDataType, uiCardNoLen, szCardNo,  uiMasterKeyIndex, uiWorkingKeyIndex, uiPinLen, szPin, uiPinLen);
    
    uiRet = iDoRemoteFunc((uchar*)psFunCall, uiDataLen, 5);
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("uiEsamCalPinBlock iDoRemoteFunc ret = %d", uiRet);
        return 99;
    }
    memcpy(psPinBlock, psGetParaBuf(8), 8);
    
    return iGetParaInt(0);
}

uint _uiRcEsamMasterKeyEncrypt(uint uiAlgo, uint uiDataType, uint uiMasterKeyIndex, uchar *psInData, uint uiInDataLen, uchar *psOutData)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(512);
    
    uiDataLen = sprintf(psFunCall, "%u(%u, %u, %u, b%03u",
                        FUNC_uiEsamMasterKeyEncrypt, uiAlgo, uiDataType, uiMasterKeyIndex, uiInDataLen);
    memcpy(psFunCall + uiDataLen, psInData, uiInDataLen);
    uiDataLen += uiInDataLen;
    uiDataLen += sprintf(psFunCall + uiDataLen, ", %u, b000)", uiInDataLen);
    uiRet = iDoRemoteFunc((uchar*)psFunCall, uiDataLen, 5);
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("uiEsamMasterKeyEncrypt iDoRemoteFunc ret = %d", uiRet);
        return 99;
    }
    memcpy(psOutData, psGetParaBuf(7), uiInDataLen);
    
    return iGetParaInt(0);
}

uint _uiRcEsamWorkingKeyEncrypt(uint uiAlgo, uint uiDataType, uint uiMasterKeyIndex, uint uiWorkingKeyIndex, uchar *psInData, uint uiInDataLen, uchar *psOutData)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(512);
    
    uiDataLen = sprintf(psFunCall, "%u(%u, %u, %u, %u, b%03u",
                        FUNC_uiEsamWorkingKeyEncrypt, uiAlgo, uiDataType, uiMasterKeyIndex, uiWorkingKeyIndex, uiInDataLen);
    memcpy(psFunCall + uiDataLen, psInData, uiInDataLen);
    uiDataLen += uiInDataLen;
    uiDataLen += sprintf(psFunCall + uiDataLen, ", %u, b000)", uiInDataLen);
    uiRet = iDoRemoteFunc((uchar*)psFunCall, uiDataLen, 5);
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("uiEsamWorkingKeyEncrypt iDoRemoteFunc ret = %d", uiRet);
        return 99;
    }
    memcpy(psOutData, psGetParaBuf(8), uiInDataLen);
    
    return iGetParaInt(0);
}

uint _uiRcPboc20CustomApdu(uint uiReader, uchar *psApduIn, uint uiInLen, uchar *psApduOut, uint *puiOutLen)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(512);
    
    uiDataLen = sprintf(psFunCall, "%u(%u, b%03u",
                        FUNC_uiPboc20CustomApdu, uiReader, uiInLen);
    memcpy(psFunCall + uiDataLen, psApduIn, uiInLen);
    uiDataLen += uiInLen;
    uiDataLen += sprintf(psFunCall + uiDataLen, ",%u,b000,0)", uiInLen);
    uiRet = iDoRemoteFunc((uchar*)psFunCall, uiDataLen, 8);
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("uiPboc20CustomApdu iDoRemoteFunc ret = %d", uiRet);
        return 99;
    }
    *puiOutLen = iGetParaInt(6);
    memcpy(psApduOut, psGetParaBuf(5), *puiOutLen);
    
    return iGetParaInt(0);
}

// 设置 CPU IC 卡通讯频率
// 输入参数： uiFreq         :	CARD_FREQUENCY_4MH		0
//                              CARD_FREQUENCY_6MH		1
//                              CARD_FREQUENCY_8MH		2
void _vRcSetIccFreq(uint uiFreq)
{
    uint uiDataLen, uiRet;
    
    char *psFunCall = malloc(50);
    uiDataLen = sprintf(psFunCall, "%u(%u)",FUNC_vSetIccFreq, uiFreq);
    uiRet = iDoRemoteFunc((uchar*)psFunCall, uiDataLen, 1);
    free(psFunCall);
    if (uiRet) {
        vWriteLogTxt("vSetIccFreq iDoRemoteFunc ret = %d", uiRet);
    }
}
